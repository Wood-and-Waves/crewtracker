...
- **Rebuilding a database from scratch takes four steps, and `schema.sql` is only one of them.** Create the project with *"Automatically expose new tables" OFF*, then apply `schema.sql`, `out-of-schema.sql`, and `grants.sql` in that order. Two things `pg_dump --schema=public` structurally cannot capture, both of which fail **silently** — the restore reports no errors and the database simply stops enforcing something:
  ...
  **`grants.sql` covers `service_role` since 2026-09-23, and did not before.** Supabase gave notice that **from 30 October 2026 a new table in `public` gets no automatic Data API grant** — until then `ALTER DEFAULT PRIVILEGES` handed every new table to anon, authenticated AND service_role the instant it existed, which is why this file never had to name the third one and a rebuild worked anyway. After that date a database rebuilt from `schema.sql` would come back with tables the SERVICE ROLE cannot touch — the crew clock, the digest and every admin route — failing the way everything in this file's remit fails: silently. Verified before the change that all 28 relations record an explicit `service_role` entry in `pg_class.relacl`, so the lines are read back from production rather than invented, and verified after by applying the regenerated file to dev: service_role reaches all 28 relations, `authenticated` still cannot see `day_rate` on `timecards` or `rate_cards`, and `rls.mts` stayed green. **A migration that CREATES a table must now grant it explicitly** — see `scripts/sql/migrations/README.md`; `anon` is almost never right, because this app's public routes read with the service role behind a token.
...
- **Edit Show → Day Types: the tint bar above each day is wider than the date + picker under
  it** (Dan, 2026-09-07, screenshot). The bar spans the full grid cell while the label and
...
tell which show's code you have. **`copy()` takes which text to fall back to** — the clipboard is
...
- **Scheduling (2026-07-28; in production since the 2026-08-06 cutover).** The whole workflow:
  ...
  - **Filling positions** — `FillPositionPicker`, role-filtered, warns on conflicts *within this
    organization only* — **across every day the booking covers, not just the day clicked**. That
    scope was the clicked date until 2026-09-16, which was right while a click booked one day and
    became a hole the moment the row booked the whole run (2026-09-15): somebody free on the
    Wednesday could be booked straight over a job they held for the rest of the week, silently.
    The warning names the show AND the overlapping dates (`lib/bookingConflicts.ts`, unit-tested)
    — "Booked on Harbour Point Gala — Tue 6 – Wed 7 · Booked on Westbrook Sales Meeting — Sat 10"
    — because a run is booked in one press, so "they are busy" without "on which days" gives a
    scheduler nothing to judge with. Grouped by show, days compressed, and a merely-asked booking
    reads "Pending on" rather than "Booked on": it has not taken the day off the table. Reached by tapping **Open** in a cell of the Scheduling screen (the tracker's open rows and Positions panel are gone since 2026-09-08). Since piece B it books a definition's other open days too (a checklist, one insert), and since 2026-09-08 booking somebody who DECLINED revives their own row instead of inserting a second one.
