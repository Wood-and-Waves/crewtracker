...
- **Rebuilding a database from scratch takes four steps, and `schema.sql` is only one of them.** Create the project with *"Automatically expose new tables" OFF*, then apply `ensure-rls-event-trigger.sql`, `schema.sql`, and `grants.sql` in that order. The reason is that `pg_dump` can only express privileges as GRANTs — a REVOKE is an *absence*, and on Supabase an absence gets inherited from `ALTER DEFAULT PRIVILEGES` rather than removed. Skip the first or last step and the `day_rate` lockdown silently doesn't exist in the rebuilt database. `npm run db:grants` regenerates `grants.sql` from production; re-run it after changing any privilege. Verified 2026-07-26 to reproduce production exactly, down to all 1390 column privileges.
