...
- **Inviting people is manual and loses invitations.** Raised by Dan 2026-07-27 after it bit him. Two separate problems:
  - ~~No email is sent.~~ **BUILT 2026-07-27** — `lib/inviteEmail.ts` + `app/api/invites/send/route.ts`. Sent from `noreply@contact.crewtracker.app` on creation, with the inviter's name and the company in both subject and body, and a Resend email button on each pending invite. Authorization is the RLS policy rather than a hand-written check: the invitation is read through the *caller's* session, so another organization's invite simply returns 404. A failed send never loses the invitation — the row and link still exist, and the UI says "created, but the email didn't send" and falls back to the link.
  - ~~A closed window strands the invite.~~ **BUILT 2026-07-27** — `PendingInvitesList.tsx` on the Team screen: an org admin sees every pending invitation with its expiry, can copy the link again, change the role (which rewrites the permission preset), and cancel it. Cancelling kills the link immediately. Needed no migration; the existing `invitations` policy is a single ALL command gated on `can_manage_users`.
...
- **Supabase auth emails are unbranded until the dashboard is configured.** "Confirm your email", magic-link sign-in, password reset and email-change are sent by Supabase Auth, not by the app, so they cannot be changed from this repo. Branded HTML for all four is in `docs/email-templates/`, with SMTP settings and steps in its README. **The bigger reason to apply it is not branding:** Supabase's built-in email service is rate-limited and explicitly not for production, so onboarding one company can exhaust it — and the failure presents as "the email never arrived" with nothing obviously wrong. Pointing it at Resend (same key, same verified no-reply sender as the invitation emails) removes that. Apply to production only; dev should stay on the defaults so test runs cannot send real mail.
