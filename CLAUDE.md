...
- **Inviting people is manual and loses invitations.** Raised by Dan 2026-07-27 after it bit him. Two separate problems:
  - **No email is sent.** `api/admin/create-invite` writes the row and returns a token; `InviteTeammateModal` shows the URL with a Copy button, and that is the *entire* delivery mechanism. The admin is expected to paste it into their own email or message. Fine for one person, poor for onboarding a company with six PMs. Resend is already wired and `contact.crewtracker.app` is verified, so sending the invite is not much work — the open question is whether app-sent email is actually wanted, given it can land in spam whereas a link pasted into a real message arrives with context.
  - **A closed window strands the invite.** The Team page fetches members only and *never loads invitations at all*, so there is no list of pending invites, no way to copy the link again, and no way for an org admin to revoke one (only the superadmin panel can). The row sits pending, invisible and unusable. Dan hit exactly this and the only recovery was querying Postgres directly. **Fix the list first, then consider email** — emailing without a list still leaves nothing recoverable or cancellable. The RLS already allows it: admins hold full access to their own org's `invitations`, so this is UI work with no schema change.
