...
// WHY
// ---
// Scheduling is built on calendar-date arithmetic, and this project has twice
...
// duplicates or drops a day. UTC has no DST, which is why addDays uses it.
...
const sentToScheduling = '2026-07-20T00:00:00Z'
...
// call — sending it to scheduling is the real transition.
check('a call still being built is New, not Staffing',
  st({ positionsTotal: 4, positionsFilled: 0 }), 'new')
check('a part-filled call AFTER being sent to scheduling is Staffing',
  st({ positionsTotal: 4, positionsFilled: 1, sent_to_scheduling_at: sentToScheduling }), 'staffing')
check('an untouched call after being sent to scheduling is Staffing',
  st({ positionsTotal: 4, positionsFilled: 0, sent_to_scheduling_at: sentToScheduling }), 'staffing')
check('a fully filled call is Pre-show once sent to scheduling',
  st({ positionsTotal: 4, positionsFilled: 4, sent_to_scheduling_at: sentToScheduling }), 'preshow')
// An admin who books everyone personally never sends it to scheduling, and the
// show is still ready — reporting it as New would be wrong.
check('fully filled without being sent to scheduling is still Pre-show',
...
check('a running show is Active even when short-staffed',
  showStatus({ start_date: '2026-07-27', end_date: '2026-07-29', positionsTotal: 6, positionsFilled: 2, sent_to_scheduling_at: sentToScheduling } as any, today), 'active')
...
check('finalized beats crewing state',
  st({ finalized_at: '2026-07-20T00:00:00Z', positionsTotal: 4, positionsFilled: 0, sent_to_scheduling_at: sentToScheduling }), 'finalized')
...
// ---------------------------------------------------------------------------
// Day scopes were tested here and are gone with the code (2026-08-03).
//
// Dan's original question — "if riggers are only needed on the first and last
// day, how do I show that?" — is now answered by ticking those two days in the
// bulk-add panel rather than choosing 'first-last' from a dropdown.
//
// The one thing worth keeping was the one-day-show case: such a show is BOTH
// the first and the last day, so a naive first-last would book two riggers for
// one day. That did not get lost; it is the de-duplication assertion in the
// addLinesTo block above, which passes [0, 0] deliberately.
// ---------------------------------------------------------------------------
